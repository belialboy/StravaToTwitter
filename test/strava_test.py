...
from src.layers.strava.src.python.utils import Utils
from unittest import mock
from unittest.mock import patch

class TestStrava(unittest.TestCase):
  @patch('src.layers.strava.src.python.utils.Utils.getEnv')
  @patch('src.layers.strava.src.python.utils.Utils.getSSM')
  ...
  def test_recoveryTime(self,getSSM,getEnv):
    ...
    with mock.patch.object(Strava, '_getAthleteFromDDB') as mock_method:
      ...
      mock_method.return_value = {"tokens": json.dumps(tokens)}
      strava=Strava(athleteId = 1234567)
      self.assertEqual(strava.getRecoveryTime({'average_heartrate':120,'elapsed_time':60*60}),Utils.secsToStr(129600))
      self.assertEqual(strava.getRecoveryTime({'average_heartrate':180,'elapsed_time':60*60*3}),Utils.secsToStr(345600))
